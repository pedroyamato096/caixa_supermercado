...
def exibir_menu_atendimento(cliente: Cliente) -> int:
    print(f"\n=== ATENDIMENTO: {cliente.nome} (ID {cliente.id}) ===")
    print(f"{OPCAO_INSERIR_ITEM} - Inserir item")
    print(f"{OPCAO_FINALIZAR_ATENDIMENTO} - Finalizar Atendimento")
    return obter_opcao_valida("Entre com a opção: ", [1, 2])


def exibir_nota_fiscal(carrinho: list, cliente: Cliente, total: float, data_hora: str):
