...
import repository.produto_repository as repo
...
def criar(nome: str, quantidade: int, preco: float) -> int:
    ...
    novo = Produto(nome=nome, quantidade=quantidade, preco=preco)
...
def atualizar(id: int, nome: str, quantidade: int, preco: float):
    ...
    _validar_dados_produto(nome, quantidade, preco)
    produto = Produto(id=id, nome=nome, quantidade=quantidade, preco=preco)
...
def descontar_estoque(id: int, quantidade: int):
    ...
    if not produto.tem_estoque_para(quantidade):
        raise ValueError(f"Estoque insuficiente. Disponível: {produto.quantidade}")
    produto.descontar(quantidade)
    repo.atualizar(produto)
