...
def _tentar_adicionar_item(carrinho: list, numero_cliente: int) -> list:
...
def _finalizar_atendimento(carrinho: list, numero_cliente: int) -> float:
    carrinho_agrupado = agrupar_carrinho(carrinho)
    total             = calcular_total_compra(carrinho_agrupado)
    data_hora         = retornar_data_hora()
    exibir_nota_fiscal(carrinho_agrupado, numero_cliente, total, data_hora)
    return total


def realizar_atendimento(numero_cliente: int) -> float:
    carrinho   = []
    em_andamento = True

    while em_andamento:
        opcao = exibir_menu_atendimento(numero_cliente)

        if opcao == OPCAO_INSERIR_ITEM:
            carrinho = _tentar_adicionar_item(carrinho, numero_cliente)
        elif opcao == OPCAO_FINALIZAR_ATENDIMENTO:
            em_andamento = False
            return _finalizar_atendimento(carrinho, numero_cliente)
        else:
